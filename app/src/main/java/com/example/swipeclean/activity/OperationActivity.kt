...
class OperationActivity : AppCompatActivity() {
    ...
    private fun setPhotos() {
        setPhoto(mUpImageView, mAlbum.photos[mAlbum.operatedIndex])
        val nextIndex = mAlbum.operatedIndex + 1
        if (nextIndex == mAlbum.totalCount) {
            mDownImageView.visibility = View.GONE

        } else {
            if (nextIndex < mAlbum.photos.size) {
                setPhoto(mDownImageView, mAlbum.photos[nextIndex])
            }
        }
    }
    ...
    private fun refreshTitle() {
        mDateTextView.text = mAlbum.formatData
        val completeCount: Int = mAlbum.operatedIndex
        val totalCount: Int = mAlbum.photos?.size ?: 0
        mCountTextView.text =
            String.format(
                Locale.getDefault(),
                "%d/%d",
    ...
    private fun startKeepPhoto() {
        if (mIsOperating || mAlbum.isOperated) {
            return
        }
    ...
    private fun startCancelPhoto() {
        ...
        val lastPhoto: Photo = mAlbum.photos[mAlbum.operatedIndex - 1]
        val currentPhoto: Photo = mAlbum.photos[mAlbum.operatedIndex]
    ...
    private fun doOnCompleted(operationType: OperationType) {
        val photo: Photo =
            if (operationType == OperationType.Cancel) mAlbum.photos[mAlbum.operatedIndex - 1] else mAlbum.photos[mAlbum.operatedIndex]
        when (operationType) {
        ...
        if (mAlbum.isOperated) {
            if (mAlbum.photos.stream().filter { item -> item.isDelete }.count().toInt() == 0) {
//                SwipeCleanCompletedActivity.openSwipeCleanCompletedActivity(
//                    this,
//                    getPresenter().getAlbumFormatDate()
//                )
            } else {
                openTrashActivity()
            }
